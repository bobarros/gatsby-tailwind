...
const Navbar = () => {
  ...
  const CustomLink = ({ to, children }: { to: string; children: string }) => (
    <Link onClick={() => setShow(false)} className={s.navLink} activeClassName="active-link" to={to}>
      {children}
    </Link>
  );
...
const s = {
  ...
  navLinks:
    "h-0 overflow-hidden flex flex-col transition-tvar lg:h-auto lg:flex-row lg:items-center lg:w-full",
  navLinksShow:
    "h-96 overflow-hidden flex flex-col transition-tvar lg:h-auto lg:flex-row lg:items-center lg:w-full",
  navLink:
    "block text-center text-2xl capitalize text-textColor tracking-letterSpacing py-4 px-0 border-t border-gray-300 transition-tvar lg:p-0 lg:border-t-0 lg:mr-4 text-base",
  contactLink: "flex justify-center py-1 px-4",
  contactButton:
    "cursor-pointer appearance-none text-white bg-primary-500 border-none rounded-radius tracking-letterSpacing py-1.5 px-3 shadow-var1 transition-tvar capitalize",
